...
const AnalyzerDashboard: React.FC<Props> = ({ matchData, parseData, onReset }) => {
  return (
    <section className="py-12 px-6 max-w-6xl mx-auto space-y-8 animate-fade-in">
      {/* Header bar */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 glass-card p-6">
        ...
        <button
          onClick={onReset}
          className="px-6 py-2.5 rounded-xl glass border border-white/10 text-sm font-semibold text-slate-200 hover:text-white hover:border-brand-500/50 transition-all flex items-center justify-center gap-2"
        >
